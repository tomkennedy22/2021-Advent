...
def find_first_simultaneous_flash(data):
    current_data = list(data)
    for step in range(5000):
        (step_flash_count, current_data) = complete_one_step(current_data)
        grid_size = len(current_data) * len(current_data[0])
        if step_flash_count == grid_size:
            #print(step_flash_count)
            #print(grid_size)
            return step+1


first_step = find_first_simultaneous_flash(data)
#Answer: 348
print(first_step)